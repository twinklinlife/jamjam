# Ragdoll Archer Quest — Art Direction (시안 단계)

> 상태: **캐릭터·무기 시안 단계**. 아직 게임 코드에 적용하지 않음. 승인 후 섹션 29(Claude Code 적용 프롬프트)를 확정해 실제 통합 진행.

## 1. 프로젝트 정보
- 게임 가제: Ragdoll Archer Quest
...
- 캐릭터 수: player 1종(generic hero) + boss 2종(Stage 1 Underboss Lim, Stage 2 Captain Woo)
...
- **닉네임은 플레이어가 직접 입력**하며, 로컬 리더보드에 그 이름이 그대로 표시됨 (기존 자동 생성 `Archer####` 닉네임 방식을 텍스트 입력 UI로 교체 — 아트가 아니라 별도 구현 항목, 섹션 22-1 참조)

### 10-2. Stage 1 Boss — "Underboss Lim"
- 체형: 다른 보스보다 크고 호리호리한(slender) 비율
- 얼굴: 긴 얼굴형, 작은 눈, 안경 없음, 살짝 미소 짓는 표정
- 헤어: 짧은 머리 (Captain Woo와 동일한 길이감)
- 의상: 셔츠(소매 걷음) + 슬랙스 — 오피스 캐주얼, 안경 쓴 Captain Woo와 실루엣으로 구분되도록 색은 차분한 톤(네이비/그레이 계열) 제안
- 컨셉 톤: 마피아 조직의 "Underboss"(보스 바로 아래 오른팔) 뉘앙스 — 위협적이라기보다 능글맞고 여유로운 미소로 캐주얼하게 표현

### 10-3. Stage 2 Boss (최종 보스) — "Captain Woo"
- 체형: Underboss Lim보다 살짝 작은 키
- 얼굴: 하관이 넓은 얼굴형, 작은 눈, 검은 테 안경
- 헤어: 스포츠컷(짧고 단정한 헤어)
- 의상: 검정 티셔츠 + 청바지 — 캐주얼하지만 존재감 있는 "Captain"다운 실루엣
- 컨셉 톤: 최종 보스답게 Stage 1의 Underboss Lim보다 살짝 더 강조된 색상 채도·아이콘(장식)으로 위엄 부여
...
| file name | 용도 | 크기 | transparent | priority |
|---|---|---|---|---|
| `boss-stage1-head.png` | Underboss Lim 머리 (긴 얼굴형, 작은 눈, 미소) | 128x128 | Y | 1 |
| `boss-stage1-torso.png` | Underboss Lim 몸통 (셔츠+슬랙스, 슬림 체형) | 128x176 | Y | 1 |
| `boss-stage2-head.png` | Captain Woo 머리 (넓은 하관, 작은 눈, 검은 테 안경, 스포츠컷) | 128x128 | Y | 1 |
| `boss-stage2-torso.png` | Captain Woo 몸통 (검정 티셔츠+청바지, 살짝 작은 키) | 128x152 | Y | 1 |
...
**Boss head — Stage 2 final boss, "Captain Woo"**
```
[Common Style Prompt] +
...
**Boss torso — Stage 2 final boss, "Captain Woo"**
```
[Common Style Prompt] +
...
**Boss head — Stage 1 mid-boss, "Underboss Lim"**
```
[Common Style Prompt] +
...
**Boss torso — Stage 1 mid-boss, "Underboss Lim"**
```
[Common Style Prompt] +
...
3. **보스 head/torso (Underboss Lim → Captain Woo 순)** — base rig 스타일이 확정된 뒤 얼굴·의상만 교체해 파생 생성
4. (승인 후) 등급별 무기 파생 3종
5. (후속) 배경·방어구
...
- Stage 1/2 진입 시 `createRagdoll()`이 generic enemy 텍스처 대신 boss별 텍스처 세트(Underboss Lim / Captain Woo)를 로드하도록 분기 (구조·물리는 동일, 텍스처만 스왑)

### 22-1. 아트 범위 아님 — 별도 구현 항목 (참고용 메모)
- **플레이어 닉네임 직접 입력**: 현재 `defaultSave()`가 `Archer####`를 자동 생성하는데, 이를 MenuScene(또는 최초 실행 시)에서 텍스트 입력 UI로 교체해 플레이어가 직접 입력한 이름이 `SAVE.nickname`에 저장되고 로컬 리더보드에 그대로 표시되도록 변경 필요. 이번 아트 시안 범위가 아니라 실제 코드 통합 단계에서 별도로 진행.
